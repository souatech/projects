...
class FireStoreUtils {
  ...
  static Future<bool> isMaintenanceMode() async {
    ...
    try {
      ...
      log('[SPLASH] isMaintenanceMode error — assuming false: $e');
    }
  ...
  static Future<void> updateWallateAmount(OrderModel orderModel) async {
    ...
    // JOXMAKO: driver earns delivery fee only — no COD remittance model
    double driverAmount =
        deliveryCharges + deliveryTips + driverDeliveryTaxAmount;

    // ── JOXMAKO: Earnings + cash tracking (persisted by setOrder caller) ──
    final double vendorEarning =
        (subTotal - totalDiscount) + productTaxAmount + orderTaxAmount;
    final double totalCustomerPaid = vendorEarning +
        packagingCharge +
        packagingTaxAmount +
        platformFee +
        platformTaxAmount +
        driverAmount;
    final paymentMethod =
        (orderModel.collectedPaymentMethod ?? orderModel.paymentMethod ?? '')
            .toString()
            .toLowerCase();
    final bool isCash = paymentMethod == 'cod' ||
        paymentMethod == 'cash' ||
        paymentMethod == 'cash_on_delivery' ||
        paymentMethod == 'cashondelivery' ||
        paymentMethod.contains('espèce') ||
        paymentMethod.contains('espece');

    orderModel.driverEarning = driverAmount;
    orderModel.vendorEarning = vendorEarning;
    orderModel.adminEarning = 0.0;
    orderModel.cashCollectedAmount = isCash ? totalCustomerPaid : 0.0;
    orderModel.cashToRemit = isCash ? totalCustomerPaid : 0.0;
    orderModel.cashRemittanceStatus = isCash ? 'pending' : 'not_applicable';
    orderModel.driverPayoutStatus = 'unpaid';
    orderModel.vendorPayoutStatus = 'unpaid';
    orderModel.driverPayoutId = null;
    orderModel.vendorPayoutId = null;
  }
  ...
  static Future<List<WithdrawalModel>?> getWithdrawHistory() async {
    ...
    await fireStore
        ...
        .where('beneficiary_id', isEqualTo: Constant.userModel!.id.toString())
        .get()
        .then((value) {
      for (var element in value.docs) {
        WithdrawalModel walletTransactionModel =
    ...
    walletTransactionList.sort((a, b) {
      final aDate = a.requestedAt ?? a.paidDate ?? a.processedAt;
      final bDate = b.requestedAt ?? b.paidDate ?? b.processedAt;

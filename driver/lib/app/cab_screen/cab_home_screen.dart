// ignore_for_file: deprecated_member_use, library_prefixes, strict_top_level_inference
...
class CabHomeScreen extends StatelessWidget {
        ...
        return Scaffold(
          body: controller.isLoading.value
              ...
              : Constant.userModel?.isDocumentVerify == false &&
                  ...
                  : Column(
                      children: [
                        // Minimum deposit check désactivé — modèle gains JOXMAKO
                                              ...
                                              GetBuilder<CabHomeController>(
                                                builder: (c) =>
                                                    flutterMap.MarkerLayer(
                                                        markers: c.osmMarkers),
                                              ),
                                              GetBuilder<CabHomeController>(
                                                builder: (c) {
                                                  if (c.routePoints
                                                          .isNotEmpty &&
                                                      c.currentOrder.value.id !=
                                                          null) {
                                                    return flutterMap
                                                        .PolylineLayer(
                                                      polylines: [
                        ...
                        GetBuilder<CabHomeController>(
                          builder: (ctrl) {
                            final hasRequest =
                                ctrl.currentOrder.value.id != null &&
                                    (ctrl.currentOrder.value.status ==
                                            Constant.driverPending ||
                                        ctrl.currentOrder.value.status ==
                                            Constant.orderPlaced);
                            return hasRequest
                                ? showDriverBottomSheet(isDark, ctrl)
                                : const SizedBox.shrink();
                          },
                        ),
                        GetBuilder<CabHomeController>(
                          builder: (ctrl) => ctrl.shouldShowOrderSheet
                              ? buildOrderActionsCard(isDark, ctrl)
                              : const SizedBox.shrink(),
                        ),
                      ],
                    ),
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    "${(double.tryParse(controller.currentOrder.value.distance?.toString() ?? '0') ?? 0.0).toStringAsFixed(2)} ${Constant.distanceType}",
                    textAlign: TextAlign.start,
                    style: TextStyle(
                      fontFamily: AppThemeData.semiBold,
                      color:
  ...
  Container buildOrderActionsCard(isDark, CabHomeController controller) {
    ...
    subTotal = double.tryParse(controller.currentOrder.value.subTotal?.toString() ?? '0') ?? 0.0;
    discount = double.tryParse(controller.currentOrder.value.discount ?? '0.0') ?? 0.0;
          ...
          InkWell(
            onTap: () async {
              if (controller.currentOrder.value.status ==
                ...
                showJoxmakoPaymentDialog(Get.context!, isDark,
                    onConfirm: (method) {
                  controller.completeRideWithPayment(method);
                });
              }
            },
            child: Container(
    ...
    Get.dialog(
      Dialog(
        ...
        child: Container(
          ...
          constraints:
              BoxConstraints(maxWidth: Responsive.width(90, context)),
          child: Padding(
            padding:
  ...
  void showJoxmakoPaymentDialog(BuildContext context, bool isDark,
      {required Function(String method) onConfirm}) {

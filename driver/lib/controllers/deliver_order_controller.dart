...
class DeliverOrderController extends GetxController {
  ...
  Future<void> completedOrder({String? collectedPaymentMethod}) async {
    ShowToastDialog.showLoader("Please wait".tr);
    await AudioPlayerService.playSound(false);
    if (collectedPaymentMethod != null) {
      orderModel.value.collectedPaymentMethod = collectedPaymentMethod;
      orderModel.value.collectedByDriverId = Constant.userModel?.id;
      orderModel.value.collectedAt = Timestamp.now();
      orderModel.value.collectedPaymentStatus = 'collected';
    }

...
class OwnerHomeController extends GetxController {
  final List<StreamSubscription> _orderSubscriptions = [];
  ...
  void _listenDriverOrders(String driverId) {
    _orderSubscriptions.add(FireStoreUtils.getCabDriverOrders(driverId).listen(
      (orders) { _processCabOrders(driverId, orders); },
      onError: (e) {},
    ));
    _orderSubscriptions.add(FireStoreUtils.listenParcelOrders(driverId).listen(
      (orders) { _processParcelOrders(driverId, orders); },
      onError: (e) {},
    ));
    _orderSubscriptions.add(FireStoreUtils.getRentalOrders(driverId).listen(
      (orders) { _processRentalOrders(driverId, orders); },
      onError: (e) {},
    ));
  }
  ...
  @override
  void onClose() {
    for (final sub in _orderSubscriptions) {
      sub.cancel();
    }
    super.onClose();
  }

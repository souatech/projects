...
class OnDemandOrderDetailsController extends GetxController {
  ...
  void calculatePrice() {
    ...
    if (double.parse(onProviderOrder.value?.platformFee ?? '0.0') > 0.0) {
      for (var taxElement in onProviderOrder.value?.platformTax ?? []) {
        platformTaxAmount.value += Constant.calculateTax(
          amount: onProviderOrder.value?.platformFee ?? '0.0',
          taxModel: taxElement,
        );
      }
    }
    ...
    totalAmount.value =
        (subTotal.value) +
        double.parse(onProviderOrder.value?.platformFee ?? '0.0') +
        taxAmount.value;
  }
  ...
  Future<void> cancelBooking() async {
    ...
    if (!_canCancelBooking(order.status)) {
    ...
    try {
      ...
      if (discountType.value == "Percentage" ||
        ...
        discountAmount =
            price * (double.tryParse(discountLabel.value) ?? 0) / 100;
      } else {
        discountAmount =
            double.tryParse(
              discountLabel.value.isEmpty ? '0' : discountLabel.value,
      ...
      for (var taxElement in order.taxModel ?? []) {
        orderTaxAmount += Constant.calculateTax(
          amount: (price - discountAmount).toString(),
          taxModel: taxElement,
        );
      }
      ...
      totalAmount =
          ...
          double.parse(order.platformFee ?? '0.0') +
          taxAmount;
      ...
      if ((order.adminCommission ?? '0') != '0' &&
        ...
        if (order.adminCommissionType!.toLowerCase() == 'percentage' ||
          ...
          adminComm =
              (totalAmount * (double.tryParse(order.adminCommission!) ?? 0)) /
              100;
        } else {
  ...
  bool _canCancelBooking(String? status) {
    return status == Constant.orderPlaced || status == Constant.orderAccepted;
  }
}

...
class BookParcelController extends GetxController {
  ...
  bool get isExpress => (selectedCategory?.title ?? '').toLowerCase().contains('express');
  bool get isGP => (selectedCategory?.title ?? '').toLowerCase().contains('gp');
  ...
  void setArguments() {
    ...
    if (user != null) {
      ...
      if (name.isNotEmpty) senderNameController.value.text = name;
      if ((user.phoneNumber ?? '').isNotEmpty) senderMobileController.value.text = user.phoneNumber!;
      if ((user.countryCode ?? '').isNotEmpty) senderCountryCodeController.value.text = user.countryCode!;
      if ((user.countryISOCode ?? '').isNotEmpty) senderCountryISOCodeController.value.text = user.countryISOCode!;
    }

    if (isGP) {
      receiverLocationController.value.text = 'Paris 75018, France';
      receiverLocation.value = UserLocation(latitude: 48.8908, longitude: 2.3453);
      receiverCountryCodeController.value.text = '+33';
      receiverISOCountryCodeController.value.text = 'FR';
    }
  }

  Future<void> getParcelWeight() async {
    parcelWeight.value = await FireStoreUtils.getParcelWeight();
    if (isExpress && parcelWeight.isNotEmpty) {
      selectedWeight = parcelWeight.firstWhereOrNull(
            (w) => (w.title ?? '').toLowerCase().contains('express'),
          ) ??
          parcelWeight.first;
    }
  }
  ...
  void goToCart() {
    ...
    ParcelOrderModel order = ParcelOrderModel(
      ...
      collectionFee: isGP ? '2000.0' : null,

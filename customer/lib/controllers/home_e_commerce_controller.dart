...
class HomeECommerceController extends GetxController {
  ...
  Future<void> getData() async {
    ...
    // ignore: avoid_print
    print(
      "[SECTION_RECEIVED] screen=Ecommerce sectionId=${section?.id} sectionName=${section?.name} type=${section?.serviceType}",
    );
    // ignore: avoid_print
    print("[SECTION_LOAD] screen=Ecommerce sectionId=${section?.id}");
    ...
    _vendorSub?.cancel();
    _vendorSub = FireStoreUtils.getAllNearestRestaurant(ecommarce: true).listen(
      (event) async {
        // ignore: avoid_print
        print("=====>${event.length}");

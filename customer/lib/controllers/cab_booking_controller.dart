// ignore_for_file: library_prefixes, depend_on_referenced_packages, avoid_print, deprecated_member_use, curly_braces_in_flow_control_structures
...
class CabBookingController extends GetxController {
  ...
  StreamSubscription? _driverPosSub;
  Timer? _driverSearchTimeoutTimer;
  final RxBool driverSearchTimedOut = false.obs;

  static const Duration _driverSearchTimeout = Duration(minutes: 2);
  ...
  Future<void> initData() async {
    ...
    _seedCurrentOrderFromArguments();
    isLoading.value = false;
    log(
      '[CAB_CUSTOMER_HOME_LOADED] uid=${FireStoreUtils.getCurrentUid()} mapType=${Constant.selectedMapType}',
    );
  }

  void _seedCurrentOrderFromArguments() {
    final args = Get.arguments;
    if (args is! Map || args['cabOrderModel'] is! CabOrderModel) {
      return;
    }
    currentOrder.value = args['cabOrderModel'] as CabOrderModel;
    print("TRACKING ORDER ID: ${currentOrder.value.id}");
    print("SERVICE TYPE: cab");
    print("TARGET ROUTE: CabBookingScreen");
    ...
    _userSub = FireStoreUtils.fireStore
        ...
        .listen((userSnapshot) async {
          ...
          if (userModel.value.inProgressOrderID != null &&
            ...
            if (validRideId == null) return;
            log('[CUSTOMER_CAB_LISTENER_ACTIVE] rideId=$validRideId');
            _rideSub?.cancel();
            _rideSub = FireStoreUtils.fireStore
                .collection(CollectionName.rides)
                .doc(validRideId)
                .snapshots()
                .listen((rideSnapshot) async {
                  ...
                  log('[CUSTOMER_CAB_UPDATE_RECEIVED] rideId=${currentOrder.value.id} status=$status driverId=${currentOrder.value.driverId}');

                  if (status == Constant.driverAccepted ||
                      status == Constant.orderInTransit) {
                    _stopDriverSearchTimeout();
                    final assignedDriverId = currentOrder.value.driverId;
                    if (assignedDriverId != null && assignedDriverId.isNotEmpty) {
                      log('[CUSTOMER_CAB_DRIVER_ASSIGNED] driverId=$assignedDriverId');
                      _driverPosSub?.cancel();
                      _driverPosSub = FireStoreUtils.fireStore
                          .collection(CollectionName.users)
                      ...
                      log('[CUSTOMER_CAB_DRIVER_ASSIGNED] driverId=null — skipping _driverPosSub');
                    }
                  }

                  log('[CUSTOMER_CAB_STATUS_RENDER] status=$status');
                  if (status == Constant.orderPlaced ||
                      status == Constant.driverPending ||
                      status == Constant.driverRejected ||
                      (status == Constant.orderAccepted &&
  ...
  Future<void> refreshVehicleTypes() async {
    if (isVehicleLoading.value) return;
    isVehicleLoading.value = true;
    try {
      final vehicleList = await FireStoreUtils.getVehicleType();
      vehicleTypes.assignAll(vehicleList);
      if (vehicleTypes.isNotEmpty &&
          vehicleTypes.every(
            (vehicle) => vehicle.id != selectedVehicleType.value.id,
          )) {
        selectedVehicleType.value = vehicleTypes.first;
      }
    } catch (e) {
      log('[CAB_VEHICLE_TYPES_LOAD_ERROR] $e');
    } finally {
      isVehicleLoading.value = false;
    }
  }

  Future<void> prepareVehicleSelection() async {
  ...
  Future<void> updateRouteMarkers(UserModel driverModel) async {
    try {
      ...
      if (Constant.selectedMapType == 'osm') {
        ...
        if (order.status == Constant.driverAccepted) {
          log(
            '[CAB_DRIVER_MARKER_ADDED] status=${order.status} driverLat=$driverLat driverLng=$driverLng',
          );
          markers.addAll([
            Marker(
  ...
  Future<void> completeOrder() async {
    if (selectedPaymentMethod.value == PaymentGateway.cod.name) {
      ...
      await FireStoreUtils.cabOrderPlace(currentOrder.value).then((
        value,
      ) async {
        ShowToastDialog.showToast("Payment successfully".tr);
        // Dispatch immediately after Firestore write — same as COD flow.
        try {
          final dispatchUrl =
              '${Constant.adminUrl}cab/dispatch/${currentOrder.value.id}';
          log('[CAB_DISPATCH_API_CALLED] POST $dispatchUrl (completeOrder)');
          final dispatchResp = await http.post(
          ...
          log(
            '[CAB_DISPATCH_API_RESPONSE] status=${dispatchResp.statusCode} body=${dispatchResp.body}',
          );
        } catch (e) {
          log('[CAB_DISPATCH_API_ERROR] $e — cron will retry within 60s');
        }
        Get.back();
      });
    }
  ...
  Future<void> placeOrder() async {
    log(
      '[CAB_CONFIRM_BUTTON_CLICKED] uid=${FireStoreUtils.getCurrentUid()} sectionId=${Constant.sectionConstantModel?.id}',
    );
    ...
    log('[CAB_RIDE_CREATED] id=${orderModel.id} status=${orderModel.status}');
    log(
      '[CAB_PICKUP_LATLNG] lat=${sourceLocation.latitude} lng=${sourceLocation.longitude} name=${orderModel.sourceLocationName}',
    );
    log(
      '[CAB_DESTINATION_LATLNG] lat=${destinationLocation.latitude} lng=${destinationLocation.longitude} name=${orderModel.destinationLocationName}',
    );
    log(
      '[CAB_ZONE_ID] sectionId=${orderModel.sectionId} vehicleId=${orderModel.vehicleId}',
    );
    log("Order Model : ${orderModel.toJson()}");
    ShowToastDialog.showLoader("Please wait".tr);
    await FireStoreUtils.cabOrderPlace(orderModel);
    log(
      '[CAB_RIDE_FIRESTORE_WRITTEN] id=${orderModel.id} collection=rides status=${orderModel.status} sectionId=${orderModel.sectionId} vehicleId=${orderModel.vehicleId} rideType=${orderModel.rideType} srcLat=${sourceLocation.latitude} srcLng=${sourceLocation.longitude}',
    );

    // Immediate dispatch — POST to admin Laravel backend (not the customer API server).
    // Best-effort: if this fails, the cron retries within 60s.
    try {
      final dispatchUrl = '${Constant.adminUrl}cab/dispatch/${orderModel.id}';
      log('[CAB_DISPATCH_API_CALLED] POST $dispatchUrl');
      final dispatchResp = await http.post(
        Uri.parse(dispatchUrl),
        headers: {
          'Content-Type': 'application/json',
    ...
    ShowToastDialog.closeLoader();
    bottomSheetType.value = 'waitingForDriver';
    _startDriverSearchTimeout(orderId: orderModel.id);
  }

  void _startDriverSearchTimeout({String? orderId}) {
    final rideId = orderId ?? currentOrder.value.id;
    if (rideId == null || rideId.isEmpty) return;
    if (_driverSearchTimeoutTimer?.isActive == true) return;

    driverSearchTimedOut.value = false;
    log('[CAB_DRIVER_SEARCH_TIMER_START] rideId=$rideId');
    _driverSearchTimeoutTimer = Timer(_driverSearchTimeout, () {
      _handleDriverSearchTimeout(rideId);
    });
  }

  void _stopDriverSearchTimeout() {
    if (_driverSearchTimeoutTimer?.isActive == true) {
      log('[CAB_DRIVER_SEARCH_TIMER_STOP] rideId=${currentOrder.value.id}');
    }
    _driverSearchTimeoutTimer?.cancel();
    _driverSearchTimeoutTimer = null;
    driverSearchTimedOut.value = false;
  }

  bool _isStillWaitingForDriver(CabOrderModel order) {
    final status = order.status;
    return status == Constant.orderPlaced ||
        status == Constant.driverPending ||
        status == Constant.driverRejected ||
        (status == Constant.orderAccepted && order.driverId == null);
  }

  Future<void> _handleDriverSearchTimeout(String rideId) async {
    try {
      log('[CAB_DRIVER_SEARCH_TIMEOUT] rideId=$rideId');
      final rideDoc =
          await FireStoreUtils.fireStore
              .collection(CollectionName.rides)
              .doc(rideId)
              .get();
      if (!rideDoc.exists || rideDoc.data() == null) return;

      final latestOrder = CabOrderModel.fromJson(rideDoc.data()!);
      if (!_isStillWaitingForDriver(latestOrder)) {
        _stopDriverSearchTimeout();
      ...
      currentOrder.value = latestOrder;
      await cancelCurrentDriverSearch(showToast: false, resetUi: false);
      driverSearchTimedOut.value = true;
      bottomSheetType.value = 'waitingForDriver';
      ShowToastDialog.showToast(
      ...
      log('[CAB_DRIVER_SEARCH_TIMEOUT_ERROR] $e');
    } finally {
      _driverSearchTimeoutTimer?.cancel();
      _driverSearchTimeoutTimer = null;
    }
  ...
  Future<void> cancelCurrentDriverSearch({
    bool showToast = true,
    bool resetUi = true,
  }) async {
    ...
    if (rideId != null && rideId.isNotEmpty) {
      ...
      await _removeRideFromInProgress(rideId);
    }
    ...
    if (resetUi) {
      resetDriverSearchState(clearLocations: true);
      bottomSheetType.value = 'location';
      if (showToast) {
  ...
  Future<void> retryDriverSearch() async {
    ...
    if (previousOrderId != null && previousOrderId.isNotEmpty) {
      await _removeRideFromInProgress(previousOrderId);
    }
    driverSearchTimedOut.value = false;
  ...
  void resetDriverSearchState({bool clearLocations = false}) {
    driverSearchTimedOut.value = false;
    currentOrder.value = CabOrderModel();
    polyLines.clear();
    markers.clear();
  ...
  Future<void> _removeRideFromInProgress(String rideId) async {
    final user =
        userModel.value.id != null ? userModel.value : Constant.userModel;
    if (user == null) return;

    final currentIds =
        (user.inProgressOrderID ?? [])
            .where((id) => id.toString() != rideId)
            .toList();
    user.inProgressOrderID = currentIds;
  ...
  void onClose() {
    ...
    couponCodeTextEditController.value.dispose();
    super.onClose();
  }

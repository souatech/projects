...
<div class="page-wrapper">
    ...
    trans('lang.save')}}
                    ...
                    <a href="{{route('vendors.document')}}" class="btn btn-default"><i
                            class="fa fa-undo"></i>{{
    trans('lang.cancel')}}</a>
                </div>
            </div>
    ...
    if (!allowedTypes.includes(f.type)) {
        $(".error_top").show().html("<p>{{ trans('lang.front_image') }} {{trans('lang.image_type_validation')}}</p>");
        window.scrollTo(0, 0);
    ...
    var allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
    if (!allowedTypes.includes(f.type)) {
        $(".error_top").show().html("<p>{{ trans('lang.back_image') }} {{trans('lang.image_type_validation')}}</p>");
        window.scrollTo(0, 0);
        evt.target.value = ''; // Clear the input
        return;
    }
                        ...
                        database.collection('documents_verify').doc(id).set({
                            ...
                            await allVendor.get().then(async function (snapshotsvendor) {
                                if (snapshotsvendor.docs.length > 0) {
                                    ...
                                    if (verification) {
                                        jQuery("#data-table_processing").hide();
                                        window.location.href = "{{route('vendors.document')}}";
                                    }
                                } else {
                                    jQuery("#data-table_processing").hide();
                                    window.location.href = "{{route('vendors.document')}}";

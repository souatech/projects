...
<div class="page-wrapper">
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">Mes revenus</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{trans('lang.dashboard')}}</a></li>
                <li class="breadcrumb-item active">Mes revenus</li>
            </ol>
        </div>
        <div>
        </div>
    ...
    <div class="container-fluid">
       ...
       {{-- JOXMAKO earnings summary --}}
       <div class="row mb-3" id="jox_rev_cards" style="display:none">
           <div class="col-sm-6 col-lg-3">
               <div class="card border">
                   <div class="card-body py-3 text-center">
                       <div class="text-muted small mb-1">Revenus disponibles</div>
                       <div class="h4 text-danger mb-1" id="jox_rev_unpaid">—</div>
                       <a href="{{ route('payments.create') }}" class="btn btn-sm btn-primary mt-1">
                           <i class="fa fa-paper-plane"></i> Demander
                       </a>
                   </div>
               </div>
           </div>
           <div class="col-sm-6 col-lg-3">
               <div class="card border">
                   <div class="card-body py-3 text-center">
                       ...
                       <div class="h4 text-success mb-0" id="jox_rev_paid">—</div>
                   </div>
               </div>
           </div>
           <div class="col-sm-6 col-lg-3">
               <div class="card border">
                   <div class="card-body py-3 text-center">
                       <div class="text-muted small mb-1">Demandes en attente</div>
                       <div class="h4 mb-1" id="jox_rev_pending">—</div>
                       <a href="{{ route('payments') }}" class="btn btn-sm btn-outline-secondary mt-1">Voir</a>
                   </div>
               </div>
           </div>
       </div>
               ...
               <div class="card border">
                 <div class="card-header d-flex justify-content-between align-items-center border-0">
                   <div class="card-header-title">
                    <h3 class="text-dark-2 mb-2 h4">Mes revenus — Historique</h3>
                    <p class="mb-0 text-dark-2">Transactions liées à vos commandes et paiements</p>
                   </div>             
                 </div>
                 <div class="card-body">
...
<script>
    ...
    refCurrency.get().then(async function (snapshots) {
        ...
        // Load JOXMAKO earnings summary after currency is ready
        loadJoxmakoRevenueStats();
    });

    async function loadJoxmakoRevenueStats() {
        try {
            var vendorSnap = await database.collection('vendors').where('author', '==', vendorId).get();
            if (vendorSnap.empty) return;
            var realVendorId = vendorSnap.docs[0].data().id;

            var ordersSnap = await database.collection('vendor_orders')
                .where('vendorID', '==', realVendorId)
            ...
            ordersSnap.docs.forEach(function(doc) {
                ...
                if (d.vendor_payout_status === 'paid') paid += earning;
                else if (earning > 0) unpaid += earning;
            });

            var reqSnap = await database.collection('vendor_payout_requests')
                .where('vendor_id', '==', realVendorId)
                .where('status', '==', 'pending')
                .get().catch(function() { return { docs: [] }; });

            var sym = currentCurrency || '';
            var dec = decimal_degits || 0;
            $('#jox_rev_unpaid').text(sym + ' ' + unpaid.toFixed(dec));
            $('#jox_rev_paid').text(sym + ' ' + paid.toFixed(dec));
            $('#jox_rev_pending').text(reqSnap.docs.length);
            $('#jox_rev_cards').show();
        } catch(e) {
            console.error('loadJoxmakoRevenueStats error:', e);
        }
    }
    ...
    async function buildHTML(val) {
        ...
        if (val.payment_method) {
            ...
            if (val.payment_method == "Stripe") {
                ...
                payment_method = '<span class="badge badge-secondary" style="font-size:0.85em">Revenu commande</span>';

            } else if (val.payment_method == "wave" || val.payment_method == "Wave") {
                payment_method = '<span class="badge badge-info" style="font-size:0.85em">Paiement Wave</span>';

            } else if (val.payment_method == "orange_money" || val.payment_method == "Orange Money") {
                payment_method = '<span class="badge badge-warning" style="background:#f76b1c;color:#fff;font-size:0.85em">Paiement Orange Money</span>';

    ...
    <script>
        ...
        ref.get().then(async function(snapshots) {
            ...
            if (!snapshots || snapshots.empty || !snapshots.docs[0]) {
                ...
                console.error('[Print] Order not found: ' + id);
                return;
            }
            ...
            var authorFirstName = order.author?.firstName || '';
            var authorLastName  = order.author?.lastName  || '';
            $(".customerName").text(authorFirstName + " " + authorLastName);
        ...
        if (order.author && order.author.hasOwnProperty('email')) {
            $("#billing_email").html('<a href="mailto:' + order.author.email + '">' + order.author.email +
                '</a>');
        }

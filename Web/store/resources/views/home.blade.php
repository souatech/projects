		...
		<div class="card mb-3 business-analytics" style="display:none">
			...
			<div class="card-body">
				...
				<div class="row business-analytics_list">
                        ...
                        {{-- JOXMAKO : Revenus disponibles --}}
                        <div class="col-sm-6 col-lg-4 mb-3">
                            <div class="card card-box-with-icon bg--8 redirectionCheck" onclick="location.href='{!! route('payments.create') !!}'">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    <div class="card-box-with-content">
                                        <h2 class="text-dark-2 mb-1 h4" id="jox_unpaid">—</h2>
                                        <p class="mb-0 small text-dark-2">Revenus disponibles</p>
                                    </div>
                                    <span class="box-icon ab"><img src="{{asset('images/payment.png')}}"/></span>
                                </div>
                            </div>
                        </div>

                        {{-- JOXMAKO : Revenus payés --}}
                        <div class="col-sm-6 col-lg-4 mb-3">
                            <div class="card card-box-with-icon bg--5 redirectionCheck" onclick="location.href='{!! route('payments') !!}'">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    <div class="card-box-with-content">
                                        <h2 class="text-dark-2 mb-1 h4" id="jox_paid">—</h2>
                                        <p class="mb-0 small text-dark-2">Revenus payés</p>
                                    </div>
                        ...
                        {{-- JOXMAKO : Demandes en attente --}}
                        <div class="col-sm-6 col-lg-4 mb-3">
                            <div class="card card-box-with-icon bg--24 redirectionCheck" onclick="location.href='{!! route('payments') !!}'">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    <div class="card-box-with-content">
                                        <h2 class="text-dark-2 mb-1 h4" id="jox_pending">—</h2>
                                        <p class="mb-0 small text-dark-2">Demandes en attente</p>
                                    </div>
                                    <span class="box-icon ab"><img src="{{asset('images/dproduct.png')}}"/></span>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    async function getTotalEarnings() {
        ...
        await database.collection('vendor_orders').where('vendorID', "==", vendorId).where('status', 'in', ["Order Completed"]).get().then((orderSnapshots) => {
            orderSnapshots.docs.forEach((doc) => {
                ...
                // Total earning (JOXMAKO: use vendor_earning, not reconstructed order_total)
                let vendor_earning = parseFloat(order.vendor_earning || 0);
                totalEarning += vendor_earning;

                // Monthly graph
                if (order.createdAt) {
                    let date = order.createdAt.toDate();
                    if (date.getFullYear() === currentYear) {
                        months[date.getMonth()] += vendor_earning;
                    }
                }
            });
        });
    ...
    async function loadJoxmakoStats() {
        try {
            var unpaid = 0, paid = 0, pendingCount = 0;

            var ordersSnap = await db.collection('vendor_orders')
            ...
            var reqSnap = await db.collection('vendor_payout_requests')
                .where('vendor_id', '==', vendorId)
                .where('status', '==', 'pending')
                .get()
                .catch(function() { return { docs: [] }; });

            pendingCount = reqSnap.docs.length;

            var sym = currentCurrency || '';
            var dec = decimal_degits || 0;
            $('#jox_unpaid').text(sym + ' ' + unpaid.toFixed(dec));
            $('#jox_paid').text(sym + ' ' + paid.toFixed(dec));
            $('#jox_pending').text(pendingCount);
        } catch(e) {
            console.error('loadJoxmakoStats error:', e);
        }
    }

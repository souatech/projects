                            ...
                            <div class="vendor_payout_create-inner">
                                ...
                                <div class="vendor_fieldset" style="display:none">
                                    ...
                                    <fieldset>
                                        ...
                                        <div class="form-group row">
                                            <div class="col-11">
                                                <div id="jox_payout_info" class="py-2"></div>
                                                <a href="{{ route('withdraw-method') }}" class="btn btn-sm btn-outline-secondary mt-2">
                                                    <i class="mdi mdi-pencil mr-1"></i>Gérer ma méthode de paiement
                                                </a>
                                            </div>
                                        </div>
                                    </fieldset>
...
@section('scripts')
    ...
    <script>
        ...
        database.collection('users').doc(vendorUserId).get().then(async function(userSnapshots) {
            ...
            existingBankDetails = userData.userBankDetails || null;
            if (existingBankDetails && existingBankDetails.bankName) {
                var methodLabel = (existingBankDetails.bankName === 'orange_money' || existingBankDetails.bankName === 'Orange Money')
                    ? '<span class="badge badge-warning" style="background:#f76b1c;color:#fff">Orange Money</span>'
                    : '<span class="badge badge-info">Wave</span>';
                var lockedHtml = existingBankDetails.payout_locked
                    ? '<span class="ml-2"><i class="mdi mdi-lock text-warning"></i> Verrouillé</span>'
                    : '';
                $('#jox_payout_info').html(
                    methodLabel + lockedHtml +
                    '<div class="mt-1 font-weight-bold">' + (existingBankDetails.accountNumber || '') + '</div>' +
                    '<div class="text-muted small">' + (existingBankDetails.holderName || '') + '</div>'
                );
            } else {
                $('#jox_payout_info').html('<span class="text-muted">Non configurée — <a href="' + withdrawMethodCreateUrl + '">Configurer</a></span>');
            }
            var ref;
            if(authRole === 'vendor'){
        ...
        $(".save_vendor_btn").click(async function(e) {
            ...
            try {
                ...
                <?php if(Route::is('store')): ?>
                // JOXMAKO: preserve userBankDetails as-is (managed via Méthode de paiement page)
                var userBankDetails = existingBankDetails;

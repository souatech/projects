...
class OnDemandCheckoutController extends Controller
{
    ...
    public function proccesstopay()
    {
        ...
        if (@$ondemand_cart['cart_order']) {
            ...
            if ($ondemand_cart['cart_order']['payment_method'] == 'razorpay') {
                ...
                return view('providersService.razorpay', ['is_checkout' => 1, 'cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'authorName' => $authorName, 'amount' => $total_pay, 'razorpaySecret' => $razorpaySecret, 'razorpayKey' => $razorpayKey, 'cart_order' => $ondemand_cart['cart_order'], 'formatted_price' => $formatted_price]);
                ...
                $payment = \Paystack\Transaction::initialize([
                    ...
                    'callback_url' => route('ondemand-success')
                ]);
                ...
                $total_pay = $ondemand_cart['cart_order']['total_pay'];
                $formatted_price = $ondemand_cart['cart_order']['currencyData']['symbol'] . number_format($total_pay, $ondemand_cart['cart_order']['currencyData']['decimal_degits']);
                return view('providersService.stripe', ['is_checkout' => 1, 'cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'authorName' => $authorName, 'amount' => $total_pay, 'stripeSecret' => $stripeSecret, 'stripeKey' => $stripeKey, 'cart_order' => $ondemand_cart['cart_order'], 'formatted_price' => $formatted_price]);
                ...
                $total_pay = $ondemand_cart['cart_order']['total_pay'];
                $formatted_price = $ondemand_cart['cart_order']['currencyData']['symbol'] . number_format($total_pay, $ondemand_cart['cart_order']['currencyData']['decimal_degits']);
                return view('providersService.paypal', ['is_checkout' => 1, 'cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'authorName' => $authorName, 'amount' => $total_pay, 'paypalSecret' => $paypalSecret, 'paypalKey' => $paypalKey, 'cart_order' => $ondemand_cart['cart_order'], 'formatted_price' => $formatted_price]);
                ...
                if (isset($xendit_enable) && $xendit_enable == true) {
                    ...
                    //$currency = $ondemand_cart['cart_order']['currencyData']['code'];
                    $currency = "IDR";
                    $fail_url = route('ondemand-pay');
                    $success_url = route('ondemand-success');
            ...
            } else if ($ondemand_cart['cart_order']['payment_method'] == 'paydunya') {
                $paydunya_masterKey  = $ondemand_cart['cart_order']['paydunya_masterKey'] ?? '';
                $paydunya_privateKey = $ondemand_cart['cart_order']['paydunya_privateKey'] ?? '';
                $paydunya_publicKey  = $ondemand_cart['cart_order']['paydunya_publicKey'] ?? '';
                $paydunya_token      = $ondemand_cart['cart_order']['paydunya_token'] ?? '';
                $paydunya_isSandbox  = $ondemand_cart['cart_order']['paydunya_isSandbox'] ?? 'true';
                $total_pay           = $ondemand_cart['cart_order']['total_pay'] ?? 0;
                $return_url          = route('ondemand-success');
                $cancel_url          = route('ondemand-paydunya-cancel');
                $callback_url        = route('ondemand-paydunya-callback');
                ...
                $payload = [
                    'invoice' => [
                        ...
                        'description'  => 'OnDemand Service Payment',
                    ],
                    'store' => [
                ...
                \Log::info('PAYDUNYA ONDEMAND KEYS', [
                    'masterKey'  => $paydunya_masterKey  ? substr($paydunya_masterKey, 0, 6)  . '...' : 'EMPTY',
                    'privateKey' => $paydunya_privateKey ? substr($paydunya_privateKey, 0, 6) . '...' : 'EMPTY',
                    'publicKey'  => $paydunya_publicKey  ? substr($paydunya_publicKey, 0, 6)  . '...' : 'EMPTY',
                    'token'      => $paydunya_token      ? substr($paydunya_token, 0, 6)      . '...' : 'EMPTY',
                    'isSandbox'  => $paydunya_isSandbox,
                    'total_pay'  => $total_pay,
                    'url'        => $url,
                ]);
                try {
    ...
    public function success()
    {
        ...
        if (isset($_GET['token']) && @$ondemand_cart['cart_order']['payment_method'] === 'paydunya') {
            $urlToken    = $_GET['token'];
            $storedToken = Session::get('paydunya_ondemand_invoice_token');
            if ($storedToken && $storedToken === $urlToken) {
                ...
                try {
                    ...
                    if (isset($verifyResult['status']) && $verifyResult['status'] === 'completed') {
                        ...
                        Session::put('ondemand_cart', $ondemand_cart);
                        Session::put('success', 'Payment successful');
                        Session::forget('paydunya_ondemand_invoice_token');
                        Session::save();
                        return redirect()->route('ondemand-success');
                    }
                } catch (\Exception $e) {
                    \Log::error('PAYDUNYA ONDEMAND VERIFY ERROR: ' . $e->getMessage());
                }
            } else {
                \Log::warning('PAYDUNYA ONDEMAND TOKEN MISMATCH', ['url_token' => $urlToken ?? null, 'stored' => $storedToken]);
            }
            return redirect()->route('ondemand-checkout');
        }
        if (isset($_GET['token'])) {
    ...
    public function paydunyaOndemandCancel()
    {
        return redirect()->route('ondemand-pay');
    }

    public function paydunyaOndemandCallback(Request $request)
    {
        $data = $request->all();
        \Log::info('PAYDUNYA ONDEMAND CALLBACK', $data);
        \Log::info('PAYDUNYA ONDEMAND CALLBACK HASH: ' . ($data['data']['hash'] ?? ''));
        \Log::info('PAYDUNYA ONDEMAND CALLBACK STATUS: ' . ($data['data']['invoice']['status'] ?? ''));

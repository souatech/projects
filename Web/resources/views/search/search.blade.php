        ...
        <div class="search py-5">
            ...
            <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link active border-0 bg-light text-dark rounded" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true"><i class="feather-home mr-2"></i><span class="vendor_counts"></span></a>
                </li>
            </ul>
    ...
    async function getResults() {
        ...
        if (foodsearch != '') {
            productdata.forEach((listval) => {
                var data = listval;
                if (!groupedData[data.vendorID]) {
                    groupedData[data.vendorID] = [];
                }
                groupedData[data.vendorID].push(data);
            });
            var alldata = [];
            await Promise.all(Object.keys(groupedData).map(async (vendorID) => {
                let groupedProducts = groupedData[vendorID];
                inValidProductIds = await getUserItemLimit(vendorID);
                groupedProducts = groupedProducts.filter(product => !inValidProductIds.includes(product.id));
                alldata = alldata.concat(groupedProducts);
            }));
                ...
                for (i = 0; i < products.length; i++) {
                    ...
                    await database.collection('vendors').doc(vendorId).get().then(async function(snapshotss) {
                        if (snapshotss.exist) {
                            ...
                            if (!inValidVendors.includes(vendor_data.author)) {
            ...
            vendordata.forEach((listval) => {
                ...
                var Ans = Name.indexOf(foodsearch.toLowerCase());
                if (Ans > -1) {
                    if (!inValidVendors.includes(data.author)) {
                        vendors.push(data);
                    }
                }
            });
    ...
    function buildHTML(alldata) {
        ...
        alldata.forEach((listval) => {
            ...
            if (val != undefined) {
                ...
                html = html + '<div class=""><div class="offer-icon position-absolute free-delivery-' + val.id + '"></div><a href="' + view_vendor_details + '" class="check_vendor_' + val.id + '"><img alt="#" src="' + productStoreImage + '" onerror="this.onerror=null;this.src=\'' + placeholderImage + '\'" class="img-fluid item-img w-100"></a></div>';
                html = html + '</div>';

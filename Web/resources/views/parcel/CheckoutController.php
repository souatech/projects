...
class CheckoutController extends Controller
{
    ...
    public function proccesstopay()
    {
        ...
        } else if ($cart['payment_method'] == 'paydunya') {

    $masterKey = $cart['paydunya_master_key'];
    $privateKey = $cart['paydunya_private_key'];
    $publicKey = $cart['paydunya_public_key'];
    $token = $cart['paydunya_token'];
    $mode = $cart['paydunya_mode'];
    ...
    return back()->with('error', $result['response_text'] ?? 'PayDunya error');
}else {
            return redirect()->route('checkout');
        }
    }

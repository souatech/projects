...
<script type="text/javascript">
    ...
    "Order Placed": "{{ trans('lang.order_placed') }}",
    "Order Accepted": "{{ trans('lang.order_accepted') }}",
    "Driver Pending": "{{ trans('lang.driver_pending') }}",
    "Order Shipped": "{{ trans('lang.order_shipped') }}",
    "In Transit": "{{ trans('lang.in_transit') }}",
    "Driver Accepted": "{{ trans('lang.driver_accepted') }}"
};
...
if (
    ...
    html = html + '<div class="payment-total d-flex"><label>{{ trans("lang.order_order_status_id") }}</label><span class="price ml-auto"><span class="badge badge-success p-2">' + translatedStatus + '</span></span></div>';
    ...
    html = html + '<div class="payment-total d-flex"><label>{{ trans("lang.order_id") }}</label><span class="price ml-auto">' + order_id + '</span></div>';
               ...
               // html = html + '<div class="col-md-2 parcel_payment-box"><span class="label">Distance</span><span class="total">' + val.distance + ' KM </span></div>';
                ...
                html = html + '<div class="col-md-2 parcel_payment-box"><span class="label">{{trans('lang.weight')}}</span><span class="total">' + parcelweight + '</span></div>';
                html = html + '<div class="col-md-2 parcel_payment-box date"><span class="label">{{trans('lang.order_date')}}</span><span class="total">' + val.createdAt.toDate().toDateString() + '</span></div>';
                var paymentStatusTranslations = {
                "Pending": "{{ trans('lang.payment_pending') }}",
                "Done": "{{ trans('lang.payment_done') }}"
            };
            ...
            if (val.paymentCollectByReceiver == true) {
                ...
                html = html + '<div class="col-md-2 parcel_payment-box pay-status"><span class="label">{{trans('lang.payment_status')}}</span><span class="total">' + translatedPayment + '</span></div><div class="col-md-2 parcel_payment-box"><span class="label">{{trans('lang.weight_charge')}}</span><span class="total price"><span class="currency-symbol-left"></span>' + formatCurrency(parcelWeightCharge, currencyData) + '<span class="currency-symbol-right"></span></span></div>';
                let parcelImage = '';
                if(val.parcelImages.length > 0){
                     parcelImage += '<div class="parcel-image-wrapper d-flex flex-wrap gap-2 mt-2">';
                    val.parcelImages.forEach((img) => {
                ...
                html = html + '<div class="row mt-3">';
                    html = html + '<div class="col-md-12 parcel_payment-box"><span class="label">{{trans('lang.parcel_images')}}</span>'+parcelImage+'</div>'; 
                html = html + '</div>';
                if(val.driverId && val.driver){
                    let driverDetail = '';
                    driverDetail += '<div class="name">Name: '+val.driver.firstName+' '+val.driver.lastName+'</div>';
                    driverDetail += '<div class="email">Email: '+val.driver.email+'</div>';
                    html = html + '<div class="row mt-3">';
                        html = html + '<div class="col-md-12 parcel_payment-box"><span class="label">{{trans('lang.driver_details')}}</span>'+driverDetail+'</div>'; 
                    html = html + '</div>';
                }
                html = html + '</div></div></div></div></div></div>';
            }
        ...
        alldata.forEach((listval) => {
            ...
            if (val.status == "Order Cancelled" || val.status == "Driver Rejected" || val.status == "Order Rejected") {
              ...
              // html = html + '<div class="col-md-2 parcel_payment-box"><span class="label">Distance</span><span class="total">' + val.distance + ' KM </span></div>';
               ...
               html = html + '<div class="col-md-2 parcel_payment-box date"><span class="label">{{ trans("lang.date") }}</span><span class="total">' + val.createdAt.toDate().toLocaleDateString() + '</span></div>';
                if (val.paymentCollectByReceiver == true) {
                    payment = 'Pending';
                } else {
                    payment = 'Done';

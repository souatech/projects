        ...
        <div class="row">
            <div class="col-md-12 top-nav mb-3">
                <ul class="nav nav-tabsa custom-tabsa border-0 bg-white rounded overflow-hidden shadow-sm p-2 c-t-order" id="myTab" role="tablist">
                    ...
                    <li class="nav-item border-top" role="presentation">
                        <a class="nav-link border-0 text-dark py-3 active" id="accepted-tab" href="{{ route('my-bookings') }}?activeTab=accepted">
...
<script type="text/javascript">
    ...
    async function getOrderDetails() {
        bookingRef.get().then(async function(bookingSnapshots) {
            ...
            await checkCommissionModel(orderDetails.sectionId);
            ...
            $.ajax({
                ...
                success: async function(data) {
                    ...
                    if (servicePriceUnit != 'Hourly') {
                        ...
                        await updateSubscriptionTotalOrders();
    ...
    async function updateSubscriptionTotalOrders() {
        ...
        await database.collection('users').where('id', '==', userId).get().then(async function(snapshot) {
            ...
            var subscriptionTotalOrders = userDetails.subscriptionTotalOrders;
            if (subscriptionModel || commissionModel) {
                if (parseInt(subscriptionTotalOrders) != '-1') {
                    subscriptionTotalOrders = parseInt(userDetails.subscriptionTotalOrders) + 1;
                }
    ...
    async function checkCommissionModel(sectionId) {
        await database.collection('sections').doc(sectionId).get().then(

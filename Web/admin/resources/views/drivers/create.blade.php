...
<script type="text/javascript">
    ...
    $(document).ready(async function () {
        ...
        refSection.get().then((snapshots) => {
            ...
            renderDriverRoleForm();
        });
        
        jQuery("#data-table_processing").hide();
    });

    function renderDriverRoleForm() {
            let html = "";
            html += `
                <div class="driver-role-wrapper">
                    <div class="form-check mb-2">
                        <input type="radio" class="form-check-input driver-role-radio" name="driver_role" id="driver_role_delivery" value="delivery" checked>
                        <label class="form-check-label" for="driver_role_delivery">Livreur</label>
                    </div>
                    <div class="form-check mb-2">
                        <input type="radio" class="form-check-input driver-role-radio" name="driver_role" id="driver_role_cab" value="cab">
                        <label class="form-check-label" for="driver_role_cab">Chauffeur</label>
                    </div>
                    <div class="form-check mb-3">
                        <input type="radio" class="form-check-input driver-role-radio" name="driver_role" id="driver_role_cab_rental" value="cab_rental">
    ...
    function renderRoleVehicleDetails() {
        ...
        sections.forEach((data) => {
            if(data.serviceTypeFlag == "cab-service" || data.serviceTypeFlag == "rental-service"){
                loadVehicleInputsForSection(data.id, data.serviceTypeFlag);
            }
        });
    }
    ...
    async function loadVehicleInputsForSection(sectionId, serviceType) {
        ...
        if (serviceType === "cab-service") {
            ...
            snap.docs.forEach((doc) => {
                const v = doc.data();
                vehicle_type.append(`<option value="${doc.id}" data-ride-type="${sectionData.rideType || ''}">${v.name}</option>`);
            });
        } else if (serviceType === "rental-service") {
            const snap = await refRentalVehicle.where("sectionId", "==", sectionId).get();
            snap.docs.forEach((doc) => {
                const v = doc.data();
                vehicle_type.append(`<option value="${doc.id}">${v.name}</option>`);
            });
        }
        ...
        carmMakeRef.docs.forEach((doc) => {
            const v = doc.data();
            car_make.append(`<option value="${v.name}">${v.name}</option>`);
        });
    }

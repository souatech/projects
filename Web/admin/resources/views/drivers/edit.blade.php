    ...
    <script type="text/javascript">
        ...
        $(document).ready(async function () {
            ...
            if (!user) {
                ...
                $(".error_top").show().html("<p>Driver not found</p>");
                window.scrollTo(0, 0);
                return;
            }
            currentDriverData = user || {};
            if (!currentDriverData.id) {
                currentDriverData.id = id;
            }
            ...
            if (user.active === true || user.isActive === true) {
            ...
            refSection.get().then((snapshots) => {
                activeSections = snapshots.docs.map((doc) => {
                    const data = doc.data();
                    data.id = data.id || doc.id;
                    return data;
                });
                renderDriverRoleForm(user);
            });

            jQuery("#data-table_processing").hide();
        });

        function inferDriverRole(user) {
            if (user.driverRole) {
                return user.driverRole;
            }
            const serviceTypes = user.serviceTypes || (user.serviceType ? [user.serviceType] : []);
            const hasCab = serviceTypes.includes("cab-service");
            const hasRental = serviceTypes.includes("rental-service");
            if (hasCab && hasRental) {
                return "cab_rental";
            }
            if (hasCab) {
        ...
        function renderDriverRoleForm(user) {
            const selectedRole = inferDriverRole(user || {});
            let html = `
                <div class="driver-role-wrapper">
                    <div class="form-check mb-2">
                        <input type="radio" class="form-check-input driver-role-radio" name="driver_role" id="driver_role_delivery" value="delivery" ${selectedRole === "delivery" ? "checked" : ""}>
                        <label class="form-check-label" for="driver_role_delivery">Livreur</label>
                    </div>
                    <div class="form-check mb-2">
                        <input type="radio" class="form-check-input driver-role-radio" name="driver_role" id="driver_role_cab" value="cab" ${selectedRole === "cab" ? "checked" : ""}>
                        <label class="form-check-label" for="driver_role_cab">Chauffeur</label>
                    </div>
                    <div class="form-check mb-3">
                        <input type="radio" class="form-check-input driver-role-radio" name="driver_role" id="driver_role_cab_rental" value="cab_rental" ${selectedRole === "cab_rental" ? "checked" : ""}>
                        <label class="form-check-label" for="driver_role_cab_rental">Chauffeur + Location</label>
                    </div>
                    <div class="text-muted mb-3">Les sections compatibles sont assignées automatiquement selon le rôle.</div>
                    <div id="role_vehicle_wrapper"></div>
                </div>
            `;
            $("#section_wrapper").html(html);
            renderRoleVehicleDetails(user || {});
        }

        function roleSections(role) {
            const deliveryFlags = ["delivery-service", "ecommerce-service", "parcel_delivery"];
            if (role === "delivery") {
                return activeSections.filter((section) => deliveryFlags.includes(section.serviceTypeFlag));
            }
            if (role === "cab") {
                return activeSections.filter((section) => section.serviceTypeFlag === "cab-service");
            }
            if (role === "cab_rental") {
                return activeSections.filter((section) => section.serviceTypeFlag === "cab-service" || section.serviceTypeFlag === "rental-service");
            }
            return [];
        }

        function renderRoleVehicleDetails(user) {
            const role = $("input[name='driver_role']:checked").val() || "delivery";
                ...
                if(data.serviceTypeFlag == "cab-service" || data.serviceTypeFlag == "rental-service"){
                    html += `
                        <div class="mb-3 p-3 border">
                            ...
                            <div class="vehicle-details pt-3" id="vehicle_details_${data.id}" data-service-type="${data.serviceTypeFlag}" data-section-name="${data.name}">
            ...
            $("#role_vehicle_wrapper").html(html);
            sections.forEach((data) => {
                if(data.serviceTypeFlag == "cab-service" || data.serviceTypeFlag == "rental-service"){
                    loadVehicleInputsForSection(data.id, data.serviceTypeFlag, user.vehicleDetails?.[data.id]);
                }
            });
        }

        $(document).on("change", ".driver-role-radio", function () {
            renderRoleVehicleDetails(currentDriverData);
        });

        async function loadVehicleInputsForSection(sectionId, serviceType, vehicleData) {
            ...
            if (vehicleData?.carModel) {
                carModelSelect.val(vehicleData.carModel);
            }
            wrapper.find(".car_number").val(vehicleData?.carPlateNumber || '');
        }

        function getSelectedSections() {
            
            const driverRole = $("input[name='driver_role']:checked").val() || "delivery";
            const selectedRoleSections = roleSections(driverRole);
            let sectionIds = selectedRoleSections.map((section) => section.id).filter(Boolean);
            let serviceTypes = [];
            let sectionNames = {};
            let vehicleDetails = {};
            let isValid = true;    

            selectedRoleSections.forEach(function (section) {
                let sectionId = section.id;
                let serviceType = section.serviceTypeFlag;
                let sectionName = section.name || section.id;
        ...
        $(".edit-form-btn").click(function () {
            ...
            if (userFirstName == '') {
                ...
                console.log('[ADMIN_DRIVER_UPDATE_BEFORE] driver=' + id + ' role=' + (currentDriverData.role || 'MISSING') + ' active=' + currentDriverData.active + ' isActive=' + currentDriverData.isActive + ' sectionIds=' + JSON.stringify(currentDriverData.sectionIds || []));

                storeImageData().then(IMG => {
                    var updatePayload = {
                        // Identity fields — always re-establish to survive GeoFirestore migration or
                        // any path that may have stored them under a nested key.
                        'id':   id,
                        'role': 'driver',
                        ...
                        // Service/section fields — the ONLY fields that change on service edit
                        'driverRole': selectedSections.driverRole,
                        ...
                        // Remove legacy single-service fields
                    ...
                    console.log('[ADMIN_DRIVER_UPDATE_PAYLOAD] driver=' + id + ' role=' + updatePayload.role + ' active=' + updatePayload.active + ' sectionIds=' + JSON.stringify(updatePayload.sectionIds) + ' serviceTypes=' + JSON.stringify(updatePayload.serviceTypes));

                    database.collection('users').doc(id).update(updatePayload).then(function (result) {
                        console.log('[ADMIN_DRIVER_UPDATE_AFTER] driver=' + id + ' success');

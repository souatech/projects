...
<script type="text/javascript">
    ...
    if (id != "") {
        ...
        var refData = database.collection('payout_requests').where('beneficiary_type', '==', 'driver').where('beneficiary_id', '==', id);
        ...
        var refData = database.collection('payout_requests').where('beneficiary_type', '==', 'driver').where('status', '==', 'pending');
        ...
        jQuery("#data-table_processing").show();
           ...
           const table = $('#example24').DataTable({
            ...
            await Promise.all(querySnapshot.docs.map(async (doc) => {
                ...
                // Normalize unified JOXMAKO fields to legacy field names used by buildHTML
                if (!childData.driverID && childData.beneficiary_id) {
                    childData.driverID = childData.beneficiary_id;
                }
                if (!childData.amount && childData.amount_requested) {
                    childData.amount = childData.amount_requested;
                }
                if (!childData.withdrawMethod && childData.payout_method) {
                    childData.withdrawMethod = childData.payout_method;
                }
                if (!childData.paymentStatus && childData.status) {
                    var s = childData.status;
                    childData.paymentStatus = s === 'pending' ? 'Pending' : s === 'paid' ? 'Success' : s === 'rejected' ? 'Reject' : s;
                }
                // Use requested_at as paidDate fallback for display
                if (!childData.paidDate && childData.requested_at) {
                    childData.paidDate = childData.requested_at;
                }

                var driver = '';

                if (childData.hasOwnProperty('beneficiary_name') && childData.beneficiary_name) {
                    driver = childData.beneficiary_name;
                } else if (childData.hasOwnProperty('driverID')) {
    ...
    function buildHTML(val) {
        ...
        if (val.withdrawMethod) {
            var _wm = val.withdrawMethod;
            var selectedwithdrawMethod = (_wm == "bank") ? "Bank Transfer"
                : (_wm == "wave" || _wm == "Wave") ? "Wave"
                : (_wm == "orange_money" || _wm == "Orange Money") ? "Orange Money"
                : _wm;
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.success == true){
                        ...
                        database.collection('payout_requests').doc(data['payoutId']).update({'paymentStatus': response.status,'payoutResponse' : response.result, 'status': response.status === 'Success' ? 'paid' : 'pending'}).then(async function (result) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.result && response.status){
                        ...
                        database.collection('payout_requests').doc(data['payoutId']).update({'paymentStatus': response.status,'payoutResponse' : response.result, 'status': response.status === 'Success' ? 'paid' : 'pending'});
    ...
    async function getPayoutDetail(payoutId) {
        ...
        var snapshot = await database.collection('payout_requests').doc(payoutId).get();
    ...
    $(document).on("click", "#submit_cancel", async function (e) {
        ...
        database.collection('payout_requests').doc(id).update({
            'status': 'rejected',
    ...
    $(document).on("click", "#submit_accept", async function (e) {
        ...
        database.collection('payout_requests').doc(id).update({
            ...
            // Marquer toutes les commandes du chauffeur comme payées (delivery + CAB)
            var reqSnap = await database.collection('payout_requests').doc(id).get();
            if (reqSnap.exists) {
                var reqData = reqSnap.data();
                var driverId = reqData.beneficiary_id || reqData.driverID || '';
                if (driverId) {
                    var batch = database.batch();

                    // Delivery (vendor_orders — champ driverID majuscule)
                    ...
                    deliverySnap.docs.forEach(function(o) {
                    ...
                    // CAB (rides — champ driverId minuscule)
                    var cabSnap = await database.collection('rides')
                        .where('driverId', '==', driverId)
                        .where('driver_payout_status', '!=', 'paid')
                        .where('status', '==', 'Order Completed')
                        .get().catch(function() { return { docs: [] }; });
                    cabSnap.docs.forEach(function(o) {
                        batch.update(database.collection('rides').doc(o.id), {
                            driver_payout_status: 'paid',
                            driver_payout_id: id,
                        });
                    });

                    // Parcel (parcel_orders — champ driverId minuscule)
                    var parcelSnap = await database.collection('parcel_orders')
                        .where('driverId', '==', driverId)
                        .where('driver_payout_status', '!=', 'paid')
                        .where('status', '==', 'Order Completed')
                        .get().catch(function() { return { docs: [] }; });
                    parcelSnap.docs.forEach(function(o) {
                        batch.update(database.collection('parcel_orders').doc(o.id), {
                            driver_payout_status: 'paid',
                            driver_payout_id: id,
                        });
                    });

                    if (deliverySnap.docs.length + cabSnap.docs.length + parcelSnap.docs.length > 0) await batch.commit();
                }
            }
        ...
        if ($('#example24 .is_open:checked').length) {
            if (confirm("{{trans('lang.selected_delete_alert')}}")) {
                ...
                $('#example24 .is_open:checked').each(function () {
                    ...
                    database.collection('payout_requests').doc(dataId).delete().then(function () {
                        setTimeout(function () {
                            window.location.reload();
                        }, 5000);
                    });
    ...
    $(document).on("click", "a[name='driver_payouts-delete']", function (e) {
        var id = this.id;
        database.collection('payout_requests').doc(id).delete().then(function () {
            window.location.reload();
        });

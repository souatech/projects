            ...
            <div class="col-12">
                <div class="d-flex top-title-section pb-4 justify-content-between">
                    <div class="d-flex top-title-left align-self-center">
                        ...
                        <h3 class="mb-0">{{trans('lang.provider_disburesement')}}</h3>
               ...
               <div class="card border">
                 <div class="card-header d-flex justify-content-between align-items-center border-0">
                   <div class="card-header-title">
                    <h3 class="text-dark-2 mb-2 h4">{{trans('lang.provider_disburesement')}}</h3>
                    <p class="mb-0 text-dark-2">{{trans('lang.provider_disburesement_table_text')}}</p>
                   </div>    
...
<script type="text/javascript">
    ...
    if (id != "") {
        ...
        var refData = database.collection('payouts').where('role','==','provider');
            ...
            await Promise.all(querySnapshot.docs.map(async (doc) => {
                ...
                childData.recid=doc.id;
                  if (childData.hasOwnProperty('vendorID')) {
                    provider = await payoutProvider(childData.vendorID);

                    if (!provider) {
                        return;
                    }
                    
                    if (selectedSectionId && provider.section_id &&  provider.section_id !== selectedSectionId) {
                        return; 
                    }

                    childData.title = provider.name;
                }
    ...
    async function buildHTML(val) {
        ...
        actionHtml = actionHtml + '<span class="action-btn" id="test">';
            ...
            if (val.paymentStatus == "In Process") {
                ...
                actionHtml = actionHtml + '<a id="' + val.id + '" name="vendor_check_status" data-auth="' + val.vendorID + '" data-amount="' + price + '" data-method="'+val.withdrawMethod+'" href="javascript:void(0)" class="btn btn-dark mb-2">{{trans('lang.check_payment_status')}}</a>';
    ...
    async function getUserData(vendorId) {
        ...
        await database.collection('users').where("id", "==", vendorId).get().then(async function (snapshotss) {
    ...
    async function payoutProvider(provider) {
        ...
        await database.collection('users').where("id", "==", provider).get().then(async function (snapshotss) {
            if (snapshotss.docs[0]) {
                ...
                providerInfo = {
                    name: payoutProvider,
                    sectionId: provider_data.section_id || null  
                };

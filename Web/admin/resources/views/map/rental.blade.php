...
<div class="page-wrapper">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var orders = [];
            var orders_drivers = [];
            database.collection('rental_orders').where('status', '==', 'In Transit').where('sectionId', '==', section_id).get().then(async function (snapshots) {
                if (snapshots.docs.length > 0) {
                    snapshots.docs.forEach((doc) => {
                        var data = doc.data();
            ...
            var drivers = [];
            database.collection('users').where('role', '==', 'driver').where('serviceType', '==', 'rental-service').where('sectionId', '==', section_id).where('location', '!=', null).get().then(async function (snapshots) {
                if (snapshots.docs.length > 0) {
                    snapshots.docs.forEach((doc) => {
                        var data = doc.data();
                        data.flag = 'available';
        ...
        async function loadData(data) {
            for (let i = 0; i < data.length; i++) {
                ...
                if(driver!='' && driver != undefined){
                    if (!driver || !driver.location || typeof driver.location.latitude !== 'number' || typeof driver.location.longitude !== 'number') {
                    ...
                    if (val.flag == "in_transit") {
                        ...
                        if (user != undefined && user!='') {
                            ...
                            var orderEditUrl = "{{ route('rental_orders.edit', ['id' => ':id']) }}";

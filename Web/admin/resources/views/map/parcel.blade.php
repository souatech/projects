...
<div class="page-wrapper">
                    ...
                    <div class="col-lg-4">
                        <div class="table-responsive ride-list">
                            <div class="form-group" id="search-box">
                                <input type="text" name="search" id="search" class="form-control" style="width:90%" placeholder="{{trans('lang.search_driver')}}er">
                            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var orders = [];
            var orders_drivers = [];
            database.collection('parcel_orders').where('status', '==', 'In Transit').where('sectionId', '==', section_id).get().then(async function (snapshots) {
                if (snapshots.docs.length > 0) {
                    snapshots.docs.forEach((doc) => {
                        var data = doc.data();
            ...
            var drivers = [];
            database.collection('users').where('role', '==', 'driver').where('serviceType', '==', 'parcel_delivery').where('sectionId', '==', section_id).where('location', '!=', null).get().then(async function (snapshots) {
                if (snapshots.docs.length > 0) {
                    snapshots.docs.forEach((doc) => {
                        var data = doc.data();
                        data.flag = 'available';
                ...
                if(driver!='' && driver != undefined){
                    ...
                    if (val.flag == "in_transit") {
                        ...
                        if (user != undefined && user!='') {
                            ...
                            var orderEditUrl = "{{ route('parcel_orders.edit', ['id' => ':id']) }}";

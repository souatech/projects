...
async function assignParcelDriverToOrder(orderId, driverId) {
    try {
        var driverRef = database.collection('users').doc(driverId);
        var orderRef  = database.collection('parcel_orders').doc(orderId);

        // Set parcel order status to Driver Pending
        await orderRef.set({ status: 'Driver Pending' }, { merge: true });

        // Add orderId to driver's orderRequestData array
        var driverSnap = await driverRef.get();
        var driverData = driverSnap.data();
        var orderRequestData = (driverData && driverData.orderRequestData) ? driverData.orderRequestData : [];
        if (orderRequestData.indexOf(orderId) === -1) {
            orderRequestData.push(orderId);
        }
        await driverRef.update({ orderRequestData: orderRequestData });

        alert('Driver assigned — parcel request sent.');
    } catch (e) {
        console.error('assignParcelDriverToOrder error:', e);
